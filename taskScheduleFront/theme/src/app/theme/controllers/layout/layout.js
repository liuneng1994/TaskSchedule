(function () {
  ...
  /** @ngInject */
  function ThemeCtrl($rootScope, $localStorage, $state, $timeout,$log,$window,$translate,sessionService) {
    $log.debug("Running  ThemeCtrl ===================================================================");
    $rootScope.settings = {
      skin: '',
      color: {
        themeprimary: '#2dc3e8',
    ...
    if (angular.isDefined($localStorage.settings) && angular.isDefined($localStorage.settings.tab)){
    ...
    $rootScope.$watch('settings.tab', function () {
      $localStorage.settings = $rootScope.settings;
    }, true);
